...
class _AddExpenseScreenState extends State<AddExpenseScreen> {
  ...
  Map<int, TextEditingController> customAmountControllers = {}; // For CUSTOM method
  ...
  final List<Map<String, dynamic>> categories = [
    ...
    {'name': 'Di chuyển', 'icon': Icons.directions_car, 'color': Color(0xFF9C27B0)},
    {'name': 'Hoạt động', 'icon': Icons.local_activity, 'color': Color(0xFF2196F3)},
    {'name': 'Shopping', 'icon': Icons.shopping_bag, 'color': Color(0xFFFFC107)},
  ...
  void _initializeData() {
    ...
    if (tripViewModel.currentTrip == null || tripViewModel.currentTrip!.id != widget.tripId) {
       tripViewModel.fetchTripDetail(widget.tripId).then((_) {
  ...
  Future<void> _handleSave() async {
    ...
    double? totalAmount = double.tryParse(_amountController.text.replaceAll(',', ''));
    ...
    if (splitMethod == 'SELECTED' && selectedSplitMembers.isEmpty) {
         ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Vui lòng chọn ít nhất 1 thành viên để chia')),
        );
        return;
    }
    ...
    if (splitMethod == 'CUSTOM') {
        ...
        if ((checksum - totalAmount).abs() > 1000) { // Tolerance 1000 VND
             ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Tổng tiền chia (${checksum.toStringAsFixed(0)}) không khớp với tổng chi phí (${totalAmount.toStringAsFixed(0)})')),
            );
            return;
        }
    }
    ...
    try {
        ...
        if (splitMethod == 'EVEN') {
            ...
            splits = tripMembers.map((m) => ExpenseSplit(
                expenseId: 0, 
                userId: m.id, 
            ...
            splits = selectedSplitMembers.map((m) => ExpenseSplit(
                expenseId: 0, 
                userId: m.id, 
                shareAmount: splitAmount, 
                sharePercentage: percentage, 
            ...
            double totalPercentage = 0;
             for (var member in tripMembers) {
                 double amount = double.tryParse(customAmountControllers[member.id]?.text ?? '0') ?? 0;
                 if (amount > 0) {
        ...
        final newExpense = Expense(
            ...
            title: _descriptionController.text.isNotEmpty ? _descriptionController.text : selectedCategory,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Thêm chi phí', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 18)),
        centerTitle: true,
        backgroundColor: Colors.white,
        elevation: 0,
        leading: IconButton(
          ...
          SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text('Giá', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.black87)),
                ...
                const Text('Nó dùng để làm gì?', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.black87)),
                const SizedBox(height: 10),
                Container(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.receipt_long_outlined, color: Colors.grey[400]),
                ...
                const Text('Phân loại', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.black87)),
                const SizedBox(height: 10),
                GridView.builder(
                    shrinkWrap: true,
                ...
                const Text('Ai đã trả tiền?', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.black87)),
                const SizedBox(height: 10),
                GestureDetector(
                            ...
                            children: [
                                if (selectedPaidBy != null)
                                    CircleAvatar(
                                        ...
                                        child: Text(
                                            selectedPaidBy!.name.isNotEmpty ? selectedPaidBy!.name[0] : '?',
                                ...
                                const SizedBox(width: 12),
                                Expanded(child: Text(selectedPaidBy?.name ?? 'Chọn người trả', style: const TextStyle(fontSize: 14))),
                ...
                const Text('Cách chia', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: Colors.black87)),
                const SizedBox(height: 10),
                _buildSplitOption(
                    title: 'Chia đều',
                ...
                child: SizedBox(
                    ...
                    child: ElevatedButton(
                        ...
                        child: isLoading 
                            ...
                            : const Text('Thêm chi phí', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Colors.white)),
                    ),
                ),
            ),
          ),
  ...
  Widget _buildSplitOption({required String title, required String subtitle, required IconData icon, required String value}) {
                  ...
                  children: [
                      ...
                      Expanded(
                          child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                  Text(title, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: isSelected ? Colors.white : Colors.black87)),
                                  const SizedBox(height: 4),
                                  Text(subtitle, style: TextStyle(fontSize: 11, color: isSelected ? Colors.white70 : Colors.grey[600])),
                              ],
                          ),
                      ),
  ...
  void _showPayerSelectionDialog() {
      ...
      final tripViewModel = Provider.of<TripViewModel>(context, listen: false);
      final members = tripViewModel.currentTrip?.members ?? [];
          ...
          builder: (context) {
              return Container(
                  ...
                  child: Column(
                      ...
                      children: [
                          const Text('Chọn người trả tiền', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                          const SizedBox(height: 10),
                          Expanded(
                              child: ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                      ...
                                      return ListTile(
                                          ...
                                          title: Text(member.name),
                                          trailing: selectedPaidBy?.id == member.id ? const Icon(Icons.check, color: Colors.blue) : null,
                                          onTap: () {
